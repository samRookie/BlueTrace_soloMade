            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <p style={{ margin: 0, fontSize: '0.875rem', color: '#64748b' }}>
                  Direct Access Mode &bull; Active Persona:{' '}
                  <strong>{user ? getRoleLabel(user.role) : ''}</strong> ({user?.email}) &bull;
                  Status: <span style={{ color: '#16a34a', fontWeight: 600 }}>{user?.status}</span>
                </p>
              </div>
          ...
          >
            ...
            <button
              ...
              style={{
                padding: '0.6rem 1.2rem',
                border: 'none',
                borderBottom: activeTab === 'demo' ? '2px solid #0284c7' : '2px solid transparent',
                background: 'none',
                  ...
                  {auditEvents.map((evt) => (
                    ...
                    >
                      ...
                      <div style={{ color: '#64748b', fontSize: '0.8rem' }}>
                        Actor: {evt.actorId || 'Anonymous'} ({evt.actorRole || 'None'}) &bull; Time:{' '}
                        {new Date(evt.createdAt).toLocaleString()} &bull; Request ID:{' '}
                        {evt.requestId || 'N/A'}
                      </div>
                    </div>
                  ))}
