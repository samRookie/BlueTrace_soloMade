...
| `dataset_metadata`         | `id`        | `evidence_id -> evidence_items.id` (CASCADE), `region_id -> regions.id`, `gis_layer_id -> gis_layers.id` | `evidence_id`            | None               | Specialized dataset attributes (format, type, frequency, access). |
